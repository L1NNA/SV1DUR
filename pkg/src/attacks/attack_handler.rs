...
use crate::primitive_types::{Address, State};
...
impl AttackHandler {
    pub fn inject(&mut self, d: &mut Device, dword_count: u32) {
    ...
    pub fn inject_command_word(&mut self, d: &mut Device, target: Address, mode_code: u8) {
        let mut w = Word::new_cmd(target, mode_code, TR::Receive);
        w.set_mode(1);
        d.write(w);
        self.end_attack();
    }
    ...
    pub fn inject_status_word(&mut self, d: &mut Device, target: Address) {
        d.write(Word::new_malicious_status(target));
        d.log(
            WRD_EMPTY,
    ...
    pub fn start_mitm(&mut self, d: &mut Device) {
        ...
        let mut w = Word::new_cmd(self.target_src, word_count, TR::Receive);
        d.write(w);
        w.set_address(self.target_dst);
        w.set_tr(TR::Transmit);
        d.write(w);
    ...
    pub fn desynchronize_rt(&mut self, d: &mut Device, target: Address) {
        ...
        let word_count = 17;
        d.write(Word::new_cmd(target, word_count, TR::Receive));
...
impl EventHandler for AttackHandler {
    fn on_cmd(&mut self, d: &mut Device, w: &mut Word) {
        match attack {
            ...
            Attack::Attack3(target) => {
                if w.address() == target && w.tr() == TR::Receive {
                    d.log(
                        *w,
            ...
            Attack::Attack4(src_target, dst_target) => {
                ...
                if self.state != State::AwtData {
                    if w.tr() == TR::Receive && w.address() == dst_target {
                        self.words_expected == w.dword_count();
                        self.state == State::AwtStsTrxR2R;
                        d.log(
                            WRD_EMPTY,
                            ErrMsg::MsgAttk(
                                format!("Atttacker>> Target dst identified (RT{})", dst_target)
                                    .to_string(),
                            ),
                        );
                    } else if w.tr() == TR::Transmit && w.address() == src_target {
                        self.state == State::AwtData;
                        d.log(
                            WRD_EMPTY,
                            ErrMsg::MsgAttk(
                                format!("Atttacker>> Target src identified (RT{})", src_target)
                                    .to_string(),
                            ),
                        );
                    }
            ...
            Attack::Attack5(target) => {
                if w.address() == target {
                    d.log(
                        WRD_EMPTY,
                        ErrMsg::MsgAttk(format!("Attacker>> Killing RT{}", target).to_string()),
                    );
                    let mode_code = 4;
                    self.inject_command_word(d, target, mode_code);
                }
            },
            Attack::Attack6(target) => {
                if self.state != State::AwtData {
                    if w.address() == target && w.tr() == TR::Receive {
                        self.words_expected = w.dword_count();
                        d.log(
                        ...
                        self.state = State::AwtData
                        self.words_expected = w.dword_count();
                        self.temp_addr = Address::from(w.address());
                        d.log(
                            WRD_EMPTY,
            ...
            Attack::Attack7(target) => {
                if w.address() == target && w.tr() == TR::Transmit {
                    d.log(
                        WRD_EMPTY,
                        ErrMsg::MsgAttk(
            ...
            Attack::Attack8(target) => {
                if w.address() == target && self.state != State::AwtData {
                    if w.tr() == TR::Receive {
                        self.words_expected = w.dword_count();
                    }
                    self.state == State::AwtData;
    ...
    fn on_sts(&mut self, d: &mut Device, w: &mut Word) {
        match attack {
            ...
            Attack::Attack4(src_target, dst_target) => {
                if self.src_target == w.address() && self.state == State::AwtData {
                    self.start_mitm(d);
                } else if self.dst_target == w.address() && self.state == State::Idle {
                    d.log(
                        WRD_EMPTY,
            ...
            Attack::Attack8(target) => {
                if self.state == State::AwtStsTrxR2R {
                    self.desynchronize_rt(d, target);
                }
            },
            _ => {}
